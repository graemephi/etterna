...
RageTextureID
Sprite::SongBGTexture(RageTextureID ID)
{
	ID.bMipMaps = true;
	...
	/* By default, song graphics are volatile: they're removed after one use.
	 * This is because some screens iteratively load and display lots of them
	 * (eg. ScreenSelectMusic,, ScreenEditMenu) one at a time, and we don't want
	 * to have hundreds of banners loaded at once. */
...
RageTextureID
Sprite::SongBannerTexture(RageTextureID ID)
{
...
void
Sprite::Load(const RageTextureID& ID)
{
...
void
Sprite::LoadFromNode(const XNode* pNode)
{
	...
	if (!sPath.empty()) {
			...
			for (auto i = 0; true; i++) {
				...
				if (iFrameIndex >= m_pTexture->GetNumFrames()) {
					LuaHelpers::ReportScriptErrorFmt(
					  "%s: State #%i is frame %d, but the texture \"%s\" only "
					  "has %d frames",
					  ActorUtil::GetWhere(pNode).c_str(),
					  i + 1,
					  iFrameIndex,
					  sPath.c_str(),
					  m_pTexture->GetNumFrames());
				}
				...
				if (pPoints[0] != nullptr && pPoints[1] != nullptr) {
					...
					newState.rect.right =
					  SCALE(fX, 0.0f, 1.0f, r.left, r.right);
					newState.rect.bottom =
					  SCALE(fY, 0.0f, 1.0f, r.top, r.bottom);
				}
		...
		if (!aStates.empty()) {
			m_States = aStates;
			Sprite::m_size.x = aStates[0].rect.GetWidth() /
							   m_pTexture->GetSourceToTexCoordsRatioX();
...
Sprite::EnableAnimation(bool bEnable)
{
	...
	if (bEnable && !bWasEnabled) {
		 ...
		 *
		 * First, and most fundamentally, the time we'll receive on the next
		 * update represents time that passed *before* the movie was started.
		 * For example, 1: 20ms passes; 2: EnableAnimation(true); 3:
		 * Update(.020). We don't want to send that 20ms to the texture; on the
		 * first update, the movie's time should be 0.
		 *
		 * Second, we don't receive Update() calls if we're in a BGAnimation
		 * that doesn't have focus. It looks like 1: EnableAnimation(false); 2:
		 * 30 seconds pass; 3: EnableAnimation(true); 4: Update(30). We must be
		 * sure not to send that long 30-second update to the movie.
		 *
...
void
Sprite::LoadFromTexture(const RageTextureID& ID)
{
...
Sprite::UpdateAnimationState()
{
	...
	if (m_States.size() > 1) {
		...
		while (m_States[m_iCurState].fDelay > min_state_delay &&
			   m_fSecsIntoState + min_state_delay >
				 m_States[m_iCurState].fDelay)
...
Sprite::DrawPrimitives()
{
	if (m_pTempState->fade.top > 0 || m_pTempState->fade.bottom > 0 ||
		...
		if (FadeSize.left > 0.001f) {
			...
			ts.crop.top +=
			  FadeDist.top; // lop off the corner if fading both x and y
			ts.crop.bottom += FadeDist.bottom;

			ts.diffuse[0] = m_pTempState->diffuse[0]; // top left -> top left
			ts.diffuse[2] =
			  m_pTempState->diffuse[2]; // bottom left -> bottom left
			ts.diffuse[3] =
			  m_pTempState->diffuse[2]; // bottom left -> bottom right
		...
		if (FadeSize.right > 0.001f) {
			...
			ts.diffuse[2] =
			  m_pTempState->diffuse[3]; // bottom right -> bottom left
			ts.diffuse[3] =
			  m_pTempState->diffuse[3]; // bottom right -> bottom right
		...
		if (FadeSize.top > 0.001f) {
			...
			ts.diffuse[3] =
			  m_pTempState->diffuse[1]; // top right -> bottom right
			ts.diffuse[1] = m_pTempState->diffuse[1]; // top right -> top right
			ts.diffuse[0].a = 0;					  // top left
		...
		if (FadeSize.bottom > 0.001f) {
			...
			ts.diffuse[3] =
			  m_pTempState->diffuse[3]; // bottom right -> bottom right
			ts.diffuse[1] =
			  m_pTempState->diffuse[3];		// bottom right -> top right
...
int
Sprite::GetNumStates() const
{
	return m_States.size();
}

void
Sprite::SetState(int iNewState)
{
	/*
			...
			if (m_pTexture)
				sError =
				  ssprintf("A Sprite '%s' (\"%s\") tried to set state to frame "
						   "%d, but it has only %u frames.",
						   /*
							* Using the state directly tends to give you an
							* error message like "tried to set frame 6 of 6"
							* which is very confusing if you don't know that one
							* is 0-indexed and the other is 1-indexed.
							* - Colby
							*/
...
void
Sprite::SetCustomTextureRect(const RectF& new_texcoord_frect)
{
...
void
Sprite::SetCustomTextureCoords(
  float fTexCoords[8]) // order: top left, bottom left, bottom right, top right
{
	m_bUsingCustomTexCoords = true;
	m_bTextureWrapping = true;
	for (auto i = 0; i < 8; i++)
...
void
Sprite::SetCustomImageCoords(float fImageCoords[8]) // order: top left, bottom
...
const RectF*
Sprite::GetCurrentTextureCoordRect() const
{
...
void
Sprite::GetActiveTextureCoords(float fTexCoordsOut[8]) const
{
...
void
Sprite::SetTexCoordVelocity(float fVelX, float fVelY)
{
...
class LunaSprite : public Luna<Sprite>
{
  public:
	...
	static int SetStateProperties(T* p, lua_State* L)
	{
		...
		// The two tables in the state are the upper left and lower right and
		// are optional.
		if (!lua_istable(L, 1)) {
		...
		for (size_t s = 0; s < num_states; ++s) {
			...
			if (lua_isnumber(L, -1) != 0) {
				frame_index = IArg(-1);
				if (frame_index < 0 ||
					frame_index >= p->GetTexture()->GetNumFrames()) {
					luaL_error(L,
							   "Frame index out of range 0-%d.",
			...
			if (lua_istable(L, -1)) {
				lua_rawgeti(L, -1, 1);
				// I have no idea why the points are from 0 to 1 and make it use
				// only a portion of the state.  This is just copied from
				// LoadFromNode. -Kyz
				new_state.rect.left =
				  SCALE(FArg(-1), 0.0f, 1.0f, r.left, r.right);
				lua_pop(L, 1);
				lua_rawgeti(L, -1, 2);
				new_state.rect.top =
				  SCALE(FArg(-1), 0.0f, 1.0f, r.top, r.bottom);
				lua_pop(L, 1);
			}
			lua_pop(L, 1);
			lua_rawgeti(L, -1, 2);
			if (lua_istable(L, -1)) {
				...
				// only a portion of the state.  This is just copied from
				// LoadFromNode. -Kyz
				new_state.rect.right =
				  SCALE(FArg(-1), 0.0f, 1.0f, r.left, r.right);
				lua_pop(L, 1);
				lua_rawgeti(L, -1, 2);
				new_state.rect.bottom =
				  SCALE(FArg(-1), 0.0f, 1.0f, r.top, r.bottom);
				lua_pop(L, 1);
			}
			lua_pop(L, 1);
